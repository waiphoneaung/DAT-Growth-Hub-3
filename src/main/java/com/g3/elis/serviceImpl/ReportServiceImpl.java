...
public class ReportServiceImpl implements ReportService {
	...
	public List<CoursePerformance> generateCoursePerformanceReport() {
		...
		for(Course course : courses)
		{
			...
			{
				if(enrolledCourse.isCompleteStatus()) totalCourseComplete++;
	...
	public List<CourseProgress> generateCourseProgressReport(int courseId) {
		...
		for(EnrolledCourse enrolledCourse : enrolledCourses)
		{
			...
			if(enrolledCourse.getUsers().getProfile() != null) courseProgress.setEnrolledUserProfileImage(enrolledCourse.getUsers().getProfile().getProfileImg());
			
			double completeEnrolledModules = 0;
			for(EnrolledModule enrolledModule : enrolledCourse.getEnrolledModules())
			{
				if(enrolledModule.isCompleteStatus() == true) completeEnrolledModules++;
			}
			courseProgress.setProgress((completeEnrolledModules/(double)(enrolledCourse.getEnrolledModules().size()))* 100);
			double totalScore = 0;
